...
int main()
{
    ...
    Shader shader("blending_1.vs","blending_1.fs");
    float cubeVertices[] = {
    ...
    glad_glGenVertexArrays(1, &grassVAO);
    glad_glGenBuffers(1, &grassVBO);
    glad_glBindVertexArray(grassVAO);
    glad_glBindBuffer(GL_ARRAY_BUFFER, grassVBO);
    glad_glBufferData(GL_ARRAY_BUFFER, sizeof(grassVertices), &grassVertices, GL_STATIC_DRAW);
    glad_glEnableVertexAttribArray(0);
    glad_glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)0);
    glad_glEnableVertexAttribArray(1);
    glad_glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)(3 * sizeof(float)));
...
//    渲染循环
    while (!glfwWindowShouldClose(window)) {
        ...
        //grass
        glad_glBindVertexArray(grassVAO);
        glad_glBindTexture(GL_TEXTURE_2D,grassTexture);
    ...
    glad_glDeleteVertexArrays(1, &grassVAO);
    glad_glDeleteBuffers(1, &cubeVBO);
    glad_glDeleteBuffers(1, &planeVBO);
    glad_glDeleteBuffers(1, &grassVBO);
...
unsigned int loadTexture( char const * path)
{
    ...
    if (data) {
        ...
        // 为当前绑定的纹理对象设置环绕、过滤方式
        glad_glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_S,format == GL_RGBA ? GL_CLAMP_TO_EDGE : GL_REPEAT);
