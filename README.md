...
##### <a href="#1"></a>  <a href="#1">1.环境搭建并创建一个窗口</a>  <a href="#2">2.画一个橙色三角形 </a>  <a href="#3">3.着色器</a>  <a href="#4">4.纹理</a>  <a href="#5">5.矩阵/变换/向量/GLM</a>  <a href="#6">6.坐标系统</a>  <a href="#7">7.FPS摄像机</a>  <a href="#8">8.颜色</a>  <a href="#9">9.基础光照</a>  <a href="#10">10.材质</a>  <a href="#11">11.光照贴图</a>  <a href="#12">12.投光物</a>  <a href="#13">13.多光源</a>  <a href="#14">14.模型加载</a>  <a href="#15">15.深度测试</a>  <a href="#16">16.模板测试</a>  <a href="#17">17.混合</a>  <a href="#18">18.面剔除</a>  <a href="#1800">关于缓冲区知识的总结和补充</a>   <a href="#19">19.帧缓冲</a>   <a href="#20">20.立方体贴图</a>    <a href="#21">21.高级数据</a>    <a href="#22">22.高级GLSL</a>    <a href="#23">23.几何着色器</a>    <a href="#24">24.实例化</a>    <a href="#25">25.抗锯齿</a>    <a href="#26">26.高级光照</a>  <a href="#27">27.Gamma校正</a> <a href="#28">28 阴影</a>  <a href="#29">29 法线贴图</a> <a href="#30">30 视差贴图</a> <a href="#31">31 HDR</a> <a href="#32">32 泛光</a> <a href="#33">33 延迟着色法</a> <a href="#34">34 SSAO</a>
...
## <a name="28_1">28.1 阴影</a>
### 定向光阴影
这章需要理解的东西挺多的，出现的新的问题也不少。这里主要是写一些遇见的问题吧，需要后续去搞明白的。
1.对于片段位置在，以光源为原点的一个坐标空间的转换。
2.把光源坐标空间系统中的片段位置，转换为裁切空间的标准化设备坐标。
3.深度贴图中的深度值为什么可以通过`float depthValue = texture(depthMap, TexCoords).r;`来获取？
纹理坐标命名为 s  t   r 和 q,和顶点坐标的 x  y  z 和 w 类似。所以当纹理类型是`GL_DEPTH_COMPONENT`时，r就从纹理的深度值。那么q是用来做什么的？q坐标对应于几何坐标的w，是一个缩放因子，作用于前三个纹理坐标。也是说实际上我们通常使用的是s/q， t/q ， r/q 。
几何坐标是x/w, y/w , z/w 。 只不过q 和 w在默认情况下为1.0。

4.深度纹理的类型为`GL_DEPTH_COMPONENT`，为什么要把高宽设置为1024？
5.在渲染循环中glViewPort的调用导致绘制的内容不能充满整个窗口。
